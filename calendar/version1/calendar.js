/**
 * Created by SunshineLXH on 2016/4/13.
 */
var ganZi = document.getElementsByClassName('ganzi')[0],
    datetable = document.getElementsByClassName('datetable')[0],
    tbody = datetable.getElementsByTagName('tbody')[0],
    select = datetable.getElementsByTagName('select');
...
select[0].onchange = changeCld;
select[1].onchange = changeCld;
...
var cld;
function drawCld(a, tM) {
    var TF = true;
    var p1 = p2 = '';
    var i, sD, s, size;
    cld = new calendar(a, tM);
    ganZi.innerHTML = '【' + Animals[(a - 4) % 12] + '】'
    for ( i = 1; i < 42; i++ ) {
        sObj = eval('SD' + i);
        lObj = eval('LD' + i);
        sObj.className = '';
        ...
        for ( var ipp = 0; ipp < sFtv.length; ipp++ ) {
            if ( parseInt(sFtv[ipp].substr(0, 2)) == (SM + 1) ) {
                if ( parseInt(sFtv[ipp].substr[0, 4]) == (sD + 1) ) {
